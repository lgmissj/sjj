function [X, Y] = LMS_train_4by2_PLL(XI,XQ,YI,YQ,TrainSeq1,TrainSeq2,delay,step,Up2,TrainLenSingle)
...
for ii = 1:TrainLen
    ...
    x3out = conj(w3)*(x3).';
    x4out = conj(w4)*(x4).';
    XoutPLL = (x1out+1j*x2out).*exp(-1j*PLL_Phase_X( (jj-1)*TrainLen+ii ));
    YoutPLL = (x3out+1j*x4out).*exp(-1j*PLL_Phase_Y( (jj-1)*TrainLen+ii ));  
