function [output1] = LMS_train_1by1(input1,TrainSeq1,delay,step,Up)
...
for jj = 1:iter
  for ii = 1:TrainLen
       x1 = [input1( (ii-1)*Up+1:(ii-1)*Up+taps )];
...
plot(abs(error1)); 
% hold on;plot(abs(error2),'r');hold off;

output1 = zeros(1,Len);
% output2 = zeros(1,Len);
for ii = 1:Len
    in1 = [input1(ii:ii+taps-1)];
